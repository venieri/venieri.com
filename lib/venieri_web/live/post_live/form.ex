defmodule VenieriWeb.PostLive.Form do
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash}>
      ...
      <.form for={@form} id="post-form" phx-change="validate" phx-submit="save">
        ...
        <.input field={@form[:logline]} type="text" label="Logline" />
        <.input field={@form[:description]} type="text" label="Description" />
        <.input field={@form[:post_date]} type="date" label="Post date" />
        <.input field={@form[:to_show]} type="checkbox" label="To show" />
        <.input
          field={@form[:orientation]}
          type="select"
          label="Orientation"
          prompt="Choose a value"
          options={Ecto.Enum.values(Venieri.Archives.Models.Post, :orientation)}
        />
        <footer>
  ...
  defp apply_action(socket, :edit, %{"id" => id}) do
    ...
    |> assign(:post, post)
    |> assign(:form, to_form(Posts.change(post)))
  end
  ...
  def handle_event("validate", %{"post" => post_params}, socket) do
    changeset = Posts.change(socket.assigns.post, post_params)
  ...
  defp save_post(socket, :edit, post_params) do
    case Posts.update(socket.assigns.post, post_params) do
      {:ok, post} ->
        {:noreply,
         socket
