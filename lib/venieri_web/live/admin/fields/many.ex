defmodule Backpex.Fields.Many do
  @config_schema [
    display_field: [
  ...
  def render_value(assigns) do
    ...
    <div class={[@live_action in [:index, :resource_action] && "truncate"]}>
      ...
      <div class={["flex", @live_action == :show && "flex-wrap"]}>
        <.intersperse
          :let={item}
          enum={@value |> Enum.sort_by(&Map.get(&1, display_field(@field)), :asc)}
        >
          <:separator>
            ,&nbsp;
          </:separator>
        ...
        <div class="dropdown w-full">
          ...
          >
            <div class="flex h-full w-full flex-wrap items-center gap-1 px-2">
              ...
              <div :for={{item, value} <- @selected} class="badge badge-sm pointer-events-auto pr-0">
                <img src={item.icon} alt={item.caption} class="w-8 h-8 mr-2" />
                ...
                <label
                  ...
                  aria-label={
                    Backpex.__({"Unselect %{item.caption}", %{label: item.caption}}, @live_resource)
                  }
                >
                  <Backpex.HTML.CoreComponents.icon
                    name="hero-x-mark"
                    class="text-primary-content size-4"
                  />
                </label>
              </div>
            </div>
          ...
          <div
            tabindex="0"
            class="dropdown-content z-[1] menu bg-base-100 rounded-box w-full overflow-y-auto shadow"
          >
            <div class="max-h-72 p-2">
              ...
              <label :if={Enum.any?(@options)}>
                <input
                  ...
                  name={
                    if @all_selected,
                      do: "change[#{@name}_deselect_all]",
                      else: "change[#{@name}_select_all]"
                  }
                  value=""
                />
  ...
  defp assign_options(socket, other_options \\ []) do
    ...
    options =
      other_options ++ options(assigns, offset: offset, limit: limit, search: search_input)
  ...
  defp options(assigns, opts) do
    ...
    display_field_form = display_field_form(field)
    ...
    |> Enum.map(fn item ->
      case display_field_form do
        fun when is_function(fun) -> {fun.(item), item.id}
        _ -> {Map.get(item, display_field_form), item.id}
      end
    end)
  end
    ...
    if String.trim(search_input) == "" do
      ...
      if select do
        ...
        where(
          ...
          ilike(field(schema_name, ^display_field), ^search_input)
        )
      end
    end
  end
  ...
  def assign_selected(socket) do
    ...
    selected_ids =
      extract_selected_ids(socket.assigns.form[socket.assigns.name].value, primary_key)
  ...
  defp fetch_selected_items(socket, selected_ids) do
    Logger.debug("Fetching selected items #{inspect(selected_ids)}")
    adapter_config = socket.assigns.live_resource.config(:adapter_config)

    %{queryable: queryable} =
      adapter_config[:schema].__schema__(:association, socket.assigns.name)

    {from_options, to_fetch} = separate_selected_items(selected_ids, socket.assigns.options)
    Logger.debug("separate_selected_items#{inspect({from_options, to_fetch})}")
    from_db = fetch_from_db(to_fetch, queryable, socket)
    Logger.debug("fetch_from_db#{inspect(from_db)}")
  ...
  defp fetch_from_db(ids_to_fetch, queryable, socket) do
    adapter_config = socket.assigns.live_resource.config(:adapter_config)
    field_form = display_field_form(socket.assigns.field)
    ...
    |> Enum.map(fn item ->
      case field_form do
        fun when is_function(fun) -> {fun.(item), item.id}
        _ -> {Map.get(item, field_form.(socket.assigns.field)), item.id}
      end
    end)
  end
